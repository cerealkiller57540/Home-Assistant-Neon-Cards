/**
 * neon-dual-thermo-card-webgl v1.0.0
 ...
 *   · liquide translucide (Beer-Lambert + émission subsurface), ménisque concave
 *     et houle de surface, bulles thermiques déformées par la traînée visqueuse ;
...
function buildConfig(raw) {
  return {
    ...
    wgl_sss:      _num(raw.wgl_sss)      ?? 0.25,  // émission subsurface : le liquide s'éclaire de l'intérieur
    wgl_plasma:   _num(raw.wgl_plasma)   ?? 2.40,  // raymarch du plasma dans le bulbe
    wgl_orbits:   _num(raw.wgl_orbits)   ?? 1.55,  // 3 orbites 3D + neutron-comète
    wgl_bubbles:  _num(raw.wgl_bubbles)  ?? 0.45,  // bulles qui montent du bulbe vers la surface
    wgl_bdeform:  _num(raw.wgl_bdeform)  ?? 0.20,  // 0 = billes sphériques, >0 = traînée visqueuse
...
class NeonDualThermoCardWebglEditor extends HTMLElement {
  ...
  _schema() {
    ...
    this._number('wgl_sss', 'Émission subsurface', { min: 0, max: 1.5, step: 0.05, ph: '0.25' });
    this._number('wgl_plasma', 'Réacteur plasma', { min: 0, max: 4, step: 0.05, ph: '2.40' });
    this._number('wgl_orbits', 'Orbites / neutrons', { min: 0, max: 3, step: 0.05, ph: '1.55' });
    this._hint('Plasma et orbites suivent aussi le bouton « Réacteur plasma » ci-dessus');
    this._number('wgl_bubbles', 'Bulles', { min: 0, max: 2, step: 0.05, ph: '0.45' });
    this._number('wgl_bdeform', 'Déformation des bulles', { min: 0, max: 1, step: 0.05, ph: '0.20' });
    this._hint('0 = billes sphériques · >0 = traînée visqueuse en filament');
...
'  for(int i=0;i<3;i++){ s+=a*n3(p); p*=2.03; a*=0.5; } return s; }',
'',
...
'',
'  // --- liquide translucide : absorption coloree + emission subsurface',
'  if (liq>0.5){',
'    float th=n.z;',
'    // Beer-Lambert : un vrai liquide colore est DENSE. Trop transparent et le',
'    // fond refracte le delave — c est ce qui faisait un liquide gris au 1er jet.',
'    vec3 absorb=exp(-(1.05-uZone)*th*4.0);',
'    float emi=0.20+uSss*0.85*pow(th,1.5);',
'    // PAS d attenuation circulaire dans le bulbe : elle peignait un CERCLE visible',
'    // dans le liquide (la demarcation que Chris voit). La visibilite du plasma est',
'    // assuree par son compositing en mix() qui REMPLACE le liquide ou il est dense.',
'    col=col*absorb*0.45 + preTone(mix(uZoneDark,uZone,pow(th,0.7)))*emi;',
'  }',
'',
...
'',
'  // --- bulles thermiques (montent du bulbe vers la surface)',
'  //     Une bulle qui monte dans un liquide dense ne reste pas spherique : la',
'  //     trainee visqueuse l etire vers le bas et la finit en filament, d autant',
'  //     plus qu elle va vite et qu elle a deja monte. On evalue donc le disque',
'  //     dans un espace ANISOTROPE (allonge vers le bas, resserre en largeur),',
'  //     ce qui donne une goutte a queue au lieu d une bille. uBDef=0 -> spheres.',
'  if (uBubbles>0.001 && dIn<0.0){',
'    for(int i=0;i<4;i++){',
'      float fi=float(i);',
'      float sd=h31(vec3(fi*13.7,3.1,7.7));',
'      float span=max(CY-surfY-2.0, 3.0);',
'      float phi=mod(t*(5.0+sd*8.0)+sd*97.0, span);',
'      float by=CY-phi;',
'      float fr=clamp(phi/span, 0.0, 1.0);          // 0 = sort du bulbe, 1 = surface',
'      float rise=(5.0+sd*8.0)/13.0;                // vitesse normalisee',
'      float bx=CX+(sd-0.5)*1.5*TWI+sin(t*1.7+fi*2.1)*1.3;',
'      float br=0.75+sd*0.85;',
'      float dx=(p.x-bx)/br, dy=(p.y-by)/br;        // y croit vers le BAS = la traine',
'      float tail=1.0+uBDef*2.8*rise*smoothstep(0.0,0.85,fr);',
'      float sy=dy>0.0 ? dy/tail : dy;',
'      float wq=1.0-uBDef*0.55*clamp(sy,0.0,1.0);   // le filament s affine en s eloignant',
'      float d=length(vec2(dx/max(wq,0.18), sy))*br;',
'      float g=smoothstep(br,br*0.3,d)*step(surfY,by);',
'      // Compositing ADDITIF (celui de thermo-webgl.html, valide par Chris) : coeur',
'      // lumineux + halo teinte zone = des etincelles de neon qui montent. La version',
'      // mix()-vers-pale + lisere sombre lisait "gouttes de lait", moins jolie.',
'      col+=(vec3(0.55)+uZone*0.55)*g*uBubbles*0.85;',
'      col+=uZone*exp(-abs(d-br)*1.9)*step(surfY,by)*uBubbles*0.18;',
'    }',
'  }',
'',
'  // --- plasma volumetrique dans le bulbe (remplace les 2 ellipses qui tournent)',
'  vec2 q=(p-vec2(CX,CY))/BRI;',
'  float r=length(q);',
'  if (r<1.0 && uPlasma>0.001){',
'    float zz=sqrt(max(1.0-r*r,0.0));',
'    float acc=0.0, wsum=0.0; vec3 pc=vec3(0.0);',
'    for(int i=0;i<6;i++){',
'      float k=(float(i)+0.5)/6.0;',
'      vec3 sp=vec3(q, mix(-zz,zz,k));',
'      // Frequence BASSE (2.6 : ~2 periodes dans le bulbe) + warp fort : c est le warp',
'      // qui etire le champ en cordes. A 4.4 on obtenait du grain, pas des filaments.',
'      vec3 w=sp*2.6+vec3(0.0,0.0,t*0.13);',
'      w+=0.90*vec3(n3(w*0.8+2.3), n3(w*0.8+11.7), n3(w*0.8+23.1));',
'      float f=fbm3(w*1.15+vec3(0.0,-t*0.22,0.0));',
'      // Bruit RIDGED : la crete est une nappe MINCE la ou f frole 0.52 — un filament.',
'      // La fenetre doit etre ETROITE : a +-0.45 le max sur 6 pas couvrait tout le',
'      // bulbe et donnait une boule cyan pleine a bord franc (vu au banc).',
'      float fil=pow(max(1.0-abs(f-0.52)*7.5,0.0), 3.0);',
'      // Projection au MAXIMUM et non moyenne : une moyenne sur 6 pas re-floute les',
'      // nappes qu on vient d affiner (meme piege que le bloom qui mange l effet).',
'      acc=max(acc,fil);',
'      pc+=mix(uP1,uP2,smoothstep(0.34,0.86,f))*fil; wsum+=fil;',
'    }',
'    pc/=max(wsum,1e-3);',
'    // Le champ meurt AVANT r=1 (jamais clippe par la porte if(r<1), sinon elle',
'    // redessine le cercle du bulbe) ET avant les orbites : le plasma est le NOYAU',
'    // de l atome, les neutrons tournent AUTOUR (r~0.67-0.86). Un coeur qui remplit',
'    // tout le bulbe les noie — verifie au banc, orbites invisibles.',
'    float fall=smoothstep(0.62,0.20,r);',
'    float w=uPlasma*(0.5+0.7*uHeat);',
'    // En ADDITIF seul, un plasma cyan pose sur un liquide jaune sature donne du BLANC :',
'    // le jaune a deja R et G a fond, ajouter G+B ne monte que le bleu. Pour lire cyan il',
'    // faut RETIRER du rouge, donc MELANGER vers la teinte du plasma — exactement ce que',
'    // fait le SVG, ou les deux ellipses sont peintes PAR-DESSUS le fill du bulbe. Ensuite',
'    // seulement on surbrille pour le neon.',
'    float fw=clamp(acc*fall*w*1.7, 0.0, 1.0);',
'    col=mix(col, preTone(pc)*1.05, fw*0.88);',
'    col+=preTone(pc)*fw*0.45 + preTone(mix(uP1,vec3(1.0),0.16))*exp(-r*2.4)*0.16*w*fall;',
'  }',
'',
...
class NeonDualThermoCardWebgl extends HTMLElement {
  ...
  _wglFrame(now) {
    ...
    rig.sides.forEach((s) => {
      ...
      gl.uniform1f(U.uOrbits,  orbits);
      gl.uniform1f(U.uBubbles, NDT_REDUCED ? 0 : c.wgl_bubbles);
