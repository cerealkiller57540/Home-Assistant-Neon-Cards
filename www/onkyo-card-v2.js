class OnkyoCard extends HTMLElement {
  constructor() {
    ...
    /* interactive controls (power / volume slider / mute / source) */
    this._interactiveHandlers = null;
    this._dragging = false;
    this._dragVol = 0;
    this._pendingVol = null;
    this._pendingVolUntil = 0;
  }

  setConfig(config) {
    if (!config.entity) throw new Error("entity requis");
    this.config = config;
    // Configuration change → rebuild the static DOM once.
    this._lastRenderKey = null;
    this._lastConfigKey = null;
    this._lastStateKey = null;
  }
  ...
  _ensureFonts() {
    ...
    if (!document.getElementById(id)) {
      ...
      link.rel = "stylesheet";
      link.href =
        "https://fonts.googleapis.com/css2?family=VT323&family=Share+Tech+Mono&family=Rajdhani:wght@400;500;600;700&display=swap";
  ...
  _render() {
    // Un drag de volume est en cours : ne pas laisser un update HA écraser
    // le feedback visuel local (le pointeur "sauterait" en arrière).
    ...
    const audioPort = !VIDEO_INVALID.has(videoPortRaw)
      ? videoPortRaw
      : audioPortRaw && audioPortRaw !== "UNKNOWN"
        ? audioPortRaw
        : "—";
    const inRes = (video.input_resolution || "—")
      .replace(/\(.*?\)/g, "")
      .trim()
      .toUpperCase();
    ...
    /*
     * ── Render pipeline ───────────────────────────────────────────────
     * The card is built only when its structure/configuration changes.
     * Normal HA state updates patch the existing DOM instead of replacing
     * shadowRoot.innerHTML. This keeps animations, focus and compositor
     * layers alive while volume/source/signal data changes.
     */
    const configKey = `${this.config.entity}|${pri}|${acc}|${thBg}|${imageUrl}|${showTexture}`;
    const stateKey = `${entity.state}|${vol}|${isMuted}|${source}|${listenMode}|${inSignal}|${inFreq}|${inCh}|${outCh}|${audioPort}|${inRes}|${hasDolby}|${hasDTS}|${Array.isArray(attr.source_list) ? attr.source_list.join("¦") : ""}`;
    const card = this.shadowRoot.querySelector("ha-card");

    if (card && configKey === this._lastConfigKey) {
      ...
      this._lastRenderKey = `${configKey}|${stateKey}`;
      return;
    }
    ...
    const inputAngle = isOn ? -SEL_SWEEP / 2 + selFrac * SEL_SWEEP : 0;
    ...
    for (let i = 0; i < srcCount; i++) {
      ...
      const [sx, sy] = polar(ang, 17),
        [ex, ey] = polar(ang, 19.5);
      const on = isOn && i === srcIdx;
      inputTicks +=
        `<line class="input-tick${on ? " active" : ""}" data-index="${i}" x1="${sx.toFixed(2)}" y1="${sy.toFixed(2)}" x2="${ex.toFixed(2)}" y2="${ey.toFixed(2)}" ` +
    ...
    const inputNeedle =
      `<line class="input-needle" x1="${SEL_CX}" y1="${SEL_CY}" x2="${nx.toFixed(2)}" y2="${ny.toFixed(2)}" ` +
...
<style>
  ...
  :host .logo { width: clamp(28px, 8vw, 40px); opacity: 0.14;}
  ...
  :host .knob-input { cursor: pointer; }
  :host .klbl {
    font-family: 'Rajdhani', sans-serif;
  ...
  :host .vbar {
    display: flex; gap: 1.5px; height: 5px;
    margin-top: 2px; padding: 4px 0; box-sizing: content-box;
    cursor: pointer; touch-action: none;
  }
  :host .vseg { flex: 1; background: rgba(255,255,255,0.03); border-radius: 0.5px; pointer-events: none; }
  :host .vseg.on {
...
<ha-card>
  <div class="fp ${thBg ? "fp-theme" : showTexture ? "fp-chassis" : "fp-metal"}">
    ...
    <div class="main">
      <div class="kwrap">
        <div class="knob">
          <svg class="ksvg" viewBox="0 0 44 44">
            ...
            <circle class="vol-arc" cx="22" cy="22" r="20" fill="none" stroke="${pri}" stroke-width="2"
      ...
      <div class="lcd${isOn ? "" : " lcd-off"}">
        ...
        <div class="r1">
          ...
          <span class="vol"><span class="vol-lbl">VOL </span><span class="vol-val">${volDisplay}</span></span>
        </div>
    ...
    /* ── Bind click + interactive controls once after DOM build ── */
    this._bindInteractive();
  }

  /* ── "more-info" quand on clique n'importe où ailleurs sur la card ── */
  _openMoreInfo() {
  ...
  /*
   * Attache le click handler global (ouvre l'intégration) + tous les
   * contrôles interactifs (power, slider volume, mute, source). Appelé une
   * seule fois après (re)construction du DOM, et ré-appelé dans
   * connectedCallback() si la shadow DOM survit (changement d'onglet).
   */
  _bindInteractive() {
    ...
    // Détache les anciens listeners avant d'en poser de nouveaux (évite les doublons).
    this._unbindInteractive();

    const h = {};
    this._interactiveHandlers = h;

    /* ── Zone "ailleurs" : ouvre l'intégration ── */
    h.card = () => this._openMoreInfo();
    card.addEventListener("click", h.card);

    /* ── ⏻ Power ── */
    const pwr = root.querySelector(".pwr");
    if (pwr) {
      h.pwr = (ev) => {
        ev.stopPropagation();
        const entity = this._hass.states[this.config.entity];
      ...
      pwr.addEventListener("click", h.pwr);
    }

    /* ── Barre de volume (32 segments) : slider souris + tactile unifié ── */
    const vbar = root.querySelector(".vbar");
    if (vbar) {
      ...
      h.vbarDown = (ev) => {
        ...
        this._dragging = true;
        this._dragVol = volFromEvent(ev);
        this._paintVolume(this._dragVol, false);
        vbar.setPointerCapture(ev.pointerId);
      };
      h.vbarMove = (ev) => {
        if (!this._dragging) return;
        ev.stopPropagation();
      ...
      const endDrag = (ev) => {
        ...
        this._dragging = false;
        try {
          vbar.releasePointerCapture(ev.pointerId);
        } catch (_) {
          /* déjà relâché */
        }
        this._commitVolume(this._dragVol);
      };
      h.vbarUp = endDrag;
      h.vbarCancel = endDrag;

      vbar.addEventListener("pointerdown", h.vbarDown);
      vbar.addEventListener("pointermove", h.vbarMove);
      vbar.addEventListener("pointerup", h.vbarUp);
      vbar.addEventListener("pointercancel", h.vbarCancel);
    }

    /* ── Knob VOL (cercle + libellé) + double-clic → mute ── */
    // Cible le knob VOL (cercle .ksvg large, cible facile) plutôt que le petit span .vol —
    // le span garde son propre stopPropagation pour ne jamais ouvrir la more-info non plus.
    const volKnob = root.querySelector(".knob:not(.knob-input)");
    const vol = root.querySelector(".vol");
    if (volKnob) {
      h.volKnobClick = (ev) => ev.stopPropagation();
      h.volKnobDbl = (ev) => {
        ev.stopPropagation();
        const entity = this._hass.states[this.config.entity];
        if (!entity || entity.state !== "on") return;
      ...
      volKnob.addEventListener("click", h.volKnobClick);
      volKnob.addEventListener("dblclick", h.volKnobDbl);
    }
    if (vol) {
      // Un simple clic sur la zone VOL ne doit jamais ouvrir la more-info
      // (sinon le 1er clic d'un double-clic la déclenche avant le dblclick).
      h.volClick = (ev) => ev.stopPropagation();
      h.volDbl = h.volKnobDbl || null;
      vol.addEventListener("click", h.volClick);
      if (h.volDbl) vol.addEventListener("dblclick", h.volDbl);
    }

    /* ── Knob INPUT : clic sur un cran → source directe, clic sur le fond → source suivante ── */
    const knobInput = root.querySelector(".knob-input");
    if (knobInput) {
      h.knobInput = (ev) => {
        ev.stopPropagation();
        const entity = this._hass.states[this.config.entity];
        if (!entity || entity.state !== "on") return;
        const srcList = Array.isArray(entity.attributes.source_list) ? entity.attributes.source_list : [];
        if (!srcList.length) return;

        /* Sélection par angle : plus fiable qu'un clic pixel-précis sur un tick de 1px.
           Même repère que le rendu (centre 22,22 dans un viewBox 0..44, 0°=midi, sens horaire, cadran ±135°). */
        const SEL_SWEEP = 270;
        const svg = knobInput.querySelector("svg");
        let nextSource = null;
        if (svg) {
          ...
          const minR = Math.min(rect.width, rect.height) * (10 / 44); // sous ce rayon : zone morte centrale, pas de cran fiable
          if (distFromCenter >= minR) {
            let angDeg = (Math.atan2(dx, -dy) * 180) / Math.PI; // 0°=midi, sens horaire
            if (angDeg >= -SEL_SWEEP / 2 - 1e-6 && angDeg <= SEL_SWEEP / 2 + 1e-6) {
              const frac = (angDeg + SEL_SWEEP / 2) / SEL_SWEEP;
              const idx = Math.round(frac * (srcList.length - 1));
              nextSource = srcList[Math.max(0, Math.min(srcList.length - 1, idx))];
            }
          }
        }
        if (!nextSource) {
          const curIdx = Math.max(0, srcList.indexOf(entity.attributes.source));
          nextSource = srcList[(curIdx + 1) % srcList.length];
        }
        if (nextSource) {
      ...
      knobInput.addEventListener("click", h.knobInput);
    }
  }

  _unbindInteractive() {
    const h = this._interactiveHandlers;
    if (!h) return;
    const root = this.shadowRoot;
    const card = root && root.querySelector("ha-card");
    if (card && h.card) card.removeEventListener("click", h.card);

    const pwr = root && root.querySelector(".pwr");
    if (pwr && h.pwr) pwr.removeEventListener("click", h.pwr);

    const vbar = root && root.querySelector(".vbar");
    if (vbar) {
      if (h.vbarDown) vbar.removeEventListener("pointerdown", h.vbarDown);
      if (h.vbarMove) vbar.removeEventListener("pointermove", h.vbarMove);
      if (h.vbarUp) vbar.removeEventListener("pointerup", h.vbarUp);
      if (h.vbarCancel) vbar.removeEventListener("pointercancel", h.vbarCancel);
    }

    const volKnob = root && root.querySelector(".knob:not(.knob-input)");
    if (volKnob) {
      if (h.volKnobClick) volKnob.removeEventListener("click", h.volKnobClick);
      if (h.volKnobDbl) volKnob.removeEventListener("dblclick", h.volKnobDbl);
    }

    const vol = root && root.querySelector(".vol");
    if (vol) {
      if (h.volClick) vol.removeEventListener("click", h.volClick);
      if (h.volDbl) vol.removeEventListener("dblclick", h.volDbl);
    }

    const knobInput = root && root.querySelector(".knob-input");
    if (knobInput && h.knobInput) knobInput.removeEventListener("click", h.knobInput);

    this._interactiveHandlers = null;
    this._clickHandler = null; // rétro-compat avec l'ancien nom utilisé ailleurs dans le fichier
  }

  /* Feedback visuel immédiat (segments + libellé VOL) sans appel réseau. */
  _paintVolume(vol, muted) {
    const root = this.shadowRoot;
    const valEl = root.querySelector(".vol-val");
    if (valEl) valEl.textContent = muted ? "MUTE" : vol;

    const segments = root.querySelectorAll(".vbar .vseg");
    segments.forEach((seg, i) => {
      const pct = (i / segments.length) * 100;
      const active = pct < vol && !muted;
      seg.classList.toggle("on", active && pct <= 80);
      seg.classList.toggle("hot", active && pct > 80);
      if (!active) seg.classList.remove("on", "hot");
    });

    const arc = root.querySelector(".vol-arc");
    if (arc) {
      const circumference = 125.66;
      const fill = !muted ? +((vol / 100) * circumference).toFixed(1) : 0;
      arc.setAttribute("stroke-dasharray", `${fill} ${(circumference - fill).toFixed(2)}`);
    }
  }

  /* Envoie la valeur finale à l'Onkyo et protège contre le rebond d'état
   * pendant le court délai où hass renvoie encore l'ancienne valeur. */
  _commitVolume(vol) {
    this._pendingVol = vol;
    this._pendingVolUntil = Date.now() + 1500;
  ...
  /*
   * Patch only dynamic values. The static faceplate, SVG logo, texture and
   * event listener remain untouched between Home Assistant state updates.
   */
  _syncCard({
    entity,
    attr,
    ...
    // Power / LCD state
    lcd.classList.toggle("lcd-off", !isOn);
    root.querySelector(".knob-input")?.classList.toggle("on", isOn);

    let standby = lcd.querySelector(".standby");
    if (!isOn && !standby) {
      standby = document.createElement("div");
      standby.className = "standby";
      standby.textContent = "STANDBY";
      lcd.appendChild(standby);
    } else if (isOn && standby) {
      standby.remove();
    }
    ...
    // Rebond post-envoi : tant que l'état HA n'a pas rattrapé la valeur que
    // l'on vient d'envoyer (delai réseau Onkyo ~200-600ms), on garde
    // l'affichage local plutôt que de repeindre l'ancienne valeur.
    let displayVol = vol;
    if (this._pendingVol !== null) {
      if (Date.now() < this._pendingVolUntil && vol !== this._pendingVol) {
        displayVol = this._pendingVol;
      } else {
        this._pendingVol = null;
      }
    }
    setText(".vol-val", isMuted ? "MUTE" : displayVol);
    setText(".mode", listenMode);
    setText(".ch", `${inCh} ▸▸ ${outCh}`);
    const specValues = root.querySelectorAll(".specs .sv");
    [inSignal, inFreq, audioPort, inRes].forEach((value, i) => {
      if (specValues[i]) specValues[i].textContent = value;
    });

    // Dolby / DTS indicators
    const fmt = root.querySelectorAll(".fmt-logo");
    const dolby = fmt[0];
    const dts = fmt[1];
    if (dolby) {
      dolby.classList.toggle("active-dolby", hasDolby && isOn);
      const path = dolby.querySelector("path");
      if (path) path.setAttribute("fill", hasDolby && isOn ? "#e02020" : "#c0c8d8");
    }
    if (dts) {
      dts.classList.toggle("active-dts", hasDTS && isOn);
      const path = dts.querySelector("path");
      if (path) path.setAttribute("fill", hasDTS && isOn ? pri : "#c0c8d8");
    }

    // Volume ring — one SVG attribute instead of rebuilding the SVG.
    const arc = root.querySelector(".vol-arc");
    if (arc) {
      const circumference = 125.66;
      const fill = isOn && !isMuted ? +((displayVol / 100) * circumference).toFixed(1) : 0;
      arc.setAttribute("stroke-dasharray", `${fill} ${(circumference - fill).toFixed(2)}`);
    }

    // 32-segment volume bar: only classes are changed.
    const segments = root.querySelectorAll(".vbar .vseg");
    segments.forEach((seg, i) => {
      const pct = (i / segments.length) * 100;
      const active = pct < displayVol && isOn && !isMuted;
      seg.classList.toggle("on", active && pct <= 80);
      seg.classList.toggle("hot", active && pct > 80);
      if (!active) {
        seg.classList.remove("on", "hot");
      }
    });

    // Input selector: update active tick + needle coordinates.
    const srcList = Array.isArray(attr.source_list) ? attr.source_list : [];
    const srcIdx = Math.max(0, srcList.indexOf(attr.source));
    const srcCount = Math.max(1, srcList.length);
    const sweep = 270;
    const frac = srcCount > 1 ? srcIdx / (srcCount - 1) : 0.5;
    const angle = isOn ? -sweep / 2 + frac * sweep : 0;
    const rad = (angle * Math.PI) / 180;
    const nx = 22 + 14 * Math.sin(rad);
    const ny = 22 - 14 * Math.cos(rad);
    const needle = root.querySelector(".input-needle");
    if (needle) {
      needle.setAttribute("x2", nx.toFixed(2));
      needle.setAttribute("y2", ny.toFixed(2));
      needle.setAttribute("stroke", isOn ? acc : "rgba(93,12,237,0.25)");
      needle.setAttribute("stroke-width", isOn ? "1.6" : "1.2");
      needle.style.filter = isOn ? `drop-shadow(0 0 3px ${acc})` : "none";
    }

    const ticks = root.querySelectorAll(".input-tick");
    if (ticks.length === srcCount) {
      ticks.forEach((tick, i) => {
        const active = isOn && i === srcIdx;
        tick.classList.toggle("active", active);
        tick.setAttribute("stroke", active ? acc : "rgba(93,12,237,0.18)");
        tick.setAttribute("stroke-width", active ? "1.4" : "1");
        tick.style.filter = active ? `drop-shadow(0 0 2px ${acc})` : "none";
      });
    }
  }
  ...
  connectedCallback() {
    // Tab switch: shadow DOM persists — just re-attach every listener.
    const card = this.shadowRoot && this.shadowRoot.querySelector("ha-card");
    if (card && this._lastRenderKey && !this._interactiveHandlers) {
      this._bindInteractive();
    }
  }
  ...
  static getConfigElement() {
    return document.createElement("onkyo-card-editor");
  }
...
customElements.define("onkyo-card-v2", OnkyoCard);
...
class OnkyoCardEditor extends HTMLElement {
  ...
  _syncValues() {
    ...
    this.querySelectorAll("[data-key]").forEach((el) => {
      ...
      if (el.type === "checkbox") {
        ...
        if (el._pick) {
          const h = this._toHex(el.value);
          if (h) el._pick.value = h;
        }
      }
    });
  }
  ...
  _render() {
    ...
    this.innerHTML = "";
    this.style.cssText =
      "display:block;padding:16px;font-family:var(--primary-font-family,Roboto,sans-serif);";
  ...
  _entityRow(key, label, value) {
    ...
    inp.type = "text";
    inp.value = value || "";
    inp.placeholder = "media_player.onkyo";
...
customElements.define("onkyo-card-editor-v2", OnkyoCardEditor);
...
console.info(
  "%c 🎵 onkyo-card v2.00 %c Neo Tokyo ",
