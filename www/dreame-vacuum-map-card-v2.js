...
function _v({ hass: u, config: f }) {
  ...
  return /* @__PURE__ */ s.jsxs("div", { ref: M, className: `dreame-vacuum-card dreame-vacuum-card--${C.name}`, children: [
    /* @__PURE__ */ s.jsxs("div", { className: "dreame-vacuum-card__container", children: [
      /* @__PURE__ */ s.jsx(v1, { entity: d, deviceName: w, onSettingsClick: () => te(!0) }),
      /* @__PURE__ */ s.jsx(
        G1,
        {
          hass: u,
      ...
      /* @__PURE__ */ s.jsx(
        A1,
        {
          cleanGeniusMode: ae(d.attributes.cleangenius_mode, ""),
          cleaningMode: ae(d.attributes.cleaning_mode, "Sweeping and mopping"),
          cleangenius: ae(d.attributes.cleangenius, "Off"),
          onClick: () => ee(!0),
          onShortcutsClick: () => ve(!0),
          disabled: ae(d.attributes.started, !1),
          language: p
        }
      ),
      /* @__PURE__ */ s.jsxs("div", { className: "dreame-vacuum-card__controls", children: [
        N === "room" && /* @__PURE__ */ s.jsx(mv, { selectedRooms: U, language: p }),
        /* @__PURE__ */ s.jsx(
          B1,
          {
            selectedMode: se,
        ...
        /* @__PURE__ */ s.jsx(
          ...
          {
            selectedMode: N,
            selectedRoomsCount: U.size,
            isRunning: ae(d.attributes.running, !1),
            isPaused: ae(d.attributes.paused, !1),
            isDocked: d.state === "docked" || ae(d.attributes.docked, !1),
            onClean: Pe,
            onPause: Ce,
            onResume: Ke,
            onStop: Xe,
    ...
    $ && /* @__PURE__ */ s.jsx(Q1, { message: $, onClose: me })
  ] });
}
...
class hv extends HTMLElement {
  ...
  constructor() {
    ...
    f.id = "dreame-card-styles";
    f.textContent = gv, this.shadowRoot.appendChild(f);
    // Card-mod support: style element to mirror card-mod injected styles
    this._cardModStyleEl = document.createElement("style");
    this._cardModStyleEl.id = "card-mod-mirror";
    this.shadowRoot.appendChild(this._cardModStyleEl);
  }
  setConfig(f) {
    if (!f.entity)
      throw new Error("You need to define an entity");
    this._config = f, this.render();
  }
  set hass(f) {
    this._hass = f, this.render();
  }
  _applyCardModStyles() {
    // Guard: prevent re-entrant calls triggered by our own DOM mutations
    ...
    try {
      ...
      if (reactRoot) {
        // Forward CSS custom properties from host context to the react root
        const props = [];
        for (let i = 0; i < hostStyles.length; i++) {
          const prop = hostStyles[i];
          if (prop.startsWith("--")) {
            props.push(`${prop}: ${hostStyles.getPropertyValue(prop)};`);
          }
        }
        const newStyle = props.join("");
        if (reactRoot.getAttribute("style") !== newStyle) {
          reactRoot.setAttribute("style", newStyle);
        }
      }
      // Collect card-mod injected <style> elements (skip our own by id)
      const cardModStyles = this.shadowRoot?.querySelectorAll('style[id*="card-mod"]') || [];
      let mirrorCSS = "";
      cardModStyles.forEach(el => {
        if (el !== this._cardModStyleEl) {
          mirrorCSS += el.textContent + "\n";
        }
      });
      if (this._cardModStyleEl && this._cardModStyleEl.textContent !== mirrorCSS) {
        this._cardModStyleEl.textContent = mirrorCSS;
      }
    } finally {
  ...
  _setupCardModObserver() {
    if (this._cardModObserver) return;
    ...
    if (this.shadowRoot) {
      // Only watch for new child nodes (style elements) added to the shadow root
      this._cardModObserver.observe(this.shadowRoot, { childList: true });
    }
    // Watch host for style/class attribute changes from card-mod
  ...
  render() {
    ...
    let f = this.shadowRoot.querySelector("#react-root");
    f || (f = document.createElement("div"), f.id = "react-root", this.shadowRoot.appendChild(f)), this._root || (this._root = tg.createRoot(f)), this._root.render(
      /* @__PURE__ */ s.jsx(Z0.StrictMode, { children: /* @__PURE__ */ s.jsx(_v, { hass: this._hass, config: this._config }) })
    );
    // Re-apply card-mod styles after render
  ...
  static getStubConfig() {
    return {
      type: "custom:dreame-vacuum-map-card-v2",
      entity: "lawn_mower.mova_1000",
      title: "Dreame Mower"
    };
  }
}
if (!customElements.get("dreame-vacuum-map-card-v2")) {
  customElements.define("dreame-vacuum-map-card-v2", hv);
}
window.customCards = window.customCards || [];
if (!window.customCards.some(c => c.type === "dreame-vacuum-map-card-v2")) {
  window.customCards.push({
    type: "dreame-vacuum-map-card-v2",
    name: "Dreame Vacuum Map Card V2",
    description: "Custom vacuum map card for Dreame vacuum cleaners (V2 with auto-detect camera entity)"
  });
}
console.info("Dreame Vacuum Map Card V2 (React) loaded");
