...
void main(){
  ...
  float breathe = 1.0 - uBreath*(0.20 - 0.20*sin(uTime*2.2*uDir));
  col   *= breathe;
  alpha *= breathe;
  ...
  function _sbcBakeSeams(modules, vbW, vbH, batOff, rangeK) {
    ...
    const best = new Float32Array(TW * TH);
    const FIELD_R = Math.max(6, Math.max(4, MODH * (rangeK || 0.5)) * sy);

    const colY = new Float32Array(TW);
  ...
  StoreyBatteryCard.prototype._startElecGL = function () {
    ...
    const loop = (now) => {
      ...
      // jusqu'au F5 (signalé par Chris le 2026-08-10 ; reproduit au probe :
...
console.info(
  "%c 🔋 storey-battery-card-gl v16.0-gl %c Neo Tokyo WebGL ",
