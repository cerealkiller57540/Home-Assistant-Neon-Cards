/**
 * id3-car-card v1.0.0
 * Version CarConnectivity MQTT (entités id3_*)
 * Fork de vw-car-card, adapté aux entités générées par l'addon Pulpyyyy/carconnectivity-addon
 */
...
class Id3CarCard extends HTMLElement {
  constructor() {
    super();
  ...
  setConfig(config) {
    ...
    this._config = {
      entity:          config.entity || 'sensor.id3_soc_primary',
      charging_entity: config.charging_entity || config.charging_state_entity || 'sensor.id3_charging_state',
      ...
      image_path:      base,
      force_refresh:   config.force_refresh || 'button.id3_wakeup',
      solar_threshold_entity: config.solar_threshold_entity || null,
      corners: {
        tl: config.corner_tl || 'lock.id3_lock_unlock',
        tr: config.corner_tr || 'binary_sensor.id3_lock_state_trunk',
        bl: config.corner_bl || 'binary_sensor.id3_door_open_state_bonnet',
        br: config.corner_br || 'binary_sensor.id3_window_open_state',
      },
      tiles: config.tiles || [
        { label: 'RANGE',   entity: 'sensor.id3_total_range',      unit: 'km', color: 'xenon' },
        { label: 'KM',      entity: 'sensor.id3_odometer',         unit: 'km', color: 'argon' },
        { label: 'POWER',   entity: 'sensor.id3_charging_power',   unit: 'W',  color: 'xenon' },
        { label: 'ÉTAT',    entity: 'sensor.id3_charging_state',   unit: '',   color: 'argon' },
      ],
      ...
      charge_switch: config.charge_switch || 'switch.id3_start_stop_charging',
  ...
  _isCharging() {
    if (this._config.charging_entity) {
      ...
      // CarConnectivity MQTT : charging, conserving, discharging, off, error...
      return cs && ['on', 'charging', 'true', 'conserving'].includes(String(cs.state).toLowerCase());
    }
    return false;
  }
  ...
  _cornerState(entity) {
    ...
    // binary_sensor CarConnectivity : state on = ouvert/problème, off = OK
    if (domain === 'binary_sensor') {
      const ok = s === 'off';
      if (entity.includes('bonnet') || entity.includes('hood')) return { ok, icon: 'mdi:car', label: ok ? 'Capot' : 'Capot !' };
      if (entity.includes('window')) return { ok, icon: ok ? 'mdi:car-door' : 'mdi:window-open-variant', label: ok ? 'Vitres' : 'Vitres !' };
      if (entity.includes('trunk') && entity.includes('lock')) {
        return { ok, icon: ok ? 'mdi:car-back' : 'mdi:car-back', label: ok ? 'Coffre' : 'Coffre !' };
      }
      if (entity.includes('door')) return { ok, icon: ok ? 'mdi:car-door-lock' : 'mdi:car-door', label: ok ? 'Portes' : 'Portes !' };
    }
...
customElements.define('id3-car-card', Id3CarCard);

window.customCards = window.customCards || [];
window.customCards.push({
  type: 'id3-car-card',
  name: 'ID.3 Car Card',
  description: 'Body voiture + batterie néon + corners status (CarConnectivity MQTT)',
});

console.info(`%c ID3-CAR-CARD %c v${VERSION}`,
