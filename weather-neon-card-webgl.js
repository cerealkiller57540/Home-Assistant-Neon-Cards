/* GENERE -- ne pas editer a la main.
   Source : weather-neon-card.js + sky_shader.py
   Build  : .preview-tooling/weather-neon-card-webgl/gen_card_webgl.py
   Toute correction faite ici sera ecrasee au prochain build. */
/**
...
function buildConfig(raw) {
  return {
    ...
    // ── nuit : un nuage EST une ombre, pas un ciel diurne timide (Chris, 2026-08-18,
    ...
    // 3.00 = choix de Chris (2026-08-18), au-dessus du 2.20 valide au banc. Raison : ce
    ...
    // Valeurs VALIDEES PAR CHRIS au banc cumul (weather_cumul_bench.html), reprises
    ...
    // neige (validee "adjuge vendu" au banc cumul). Elle n'est PAS un #define : la
    ...
    fx_neige_balanc: _n(raw.fx_neige_balanc, 1.00),
    fx_neige_prof:   _n(raw.fx_neige_prof,   3.00),
    ...
    // Shader : aurora_shader.py. Valeurs VALIDEES PAR CHRIS au banc
    ...
    // et Chris a demande de remonter le rideau de 3 px (3/234 ~ 0.013).
...
function iconSvg(condition, size, haloColor, withKanji = false) {
  ...
  if (!out) {
    ...
    out = svgWrap((comp !== undefined ? comp : fn()) + k, size, haloColor);
...
class WeatherNeonCardWebgl extends HTMLElement {
  ...
  _build() {
    ...
    // PAS d'init ici : l'aurore ne s'allume que sur verdict (cf _aurEnsure). Un
    // contexte WebGL de plus, 29 nuits sur 30 pour rien, sur un plafond de ~16.
    ...
    this._ro = new ResizeObserver(() => {
      ...
      this._frostReflow = setTimeout(() => {
        ...
        if (this._gl && this._elSkyGl && this._fitCanvas(this._elSkyGl)) {
          this._gl.viewport(0, 0, this._elSkyGl.width, this._elSkyGl.height);
  ...
  _fxTick(now) {
    ...
    if (!active) {
      ...
      if (this._fxGl) {
        this._fxGl.clearColor(0, 0, 0, 0);
        this._fxGl.clear(this._fxGl.COLOR_BUFFER_BIT);
    ...
    // La neige est peinte APRES les autres effets 2D mais AVANT la passe GL : elle
    // fait partie de la scene qu'on regarde a travers la vitre, pas de la vitre.
    // Clip zone hero, comme vent/pluie : elle ne doit pas neiger sur les previsions.
    if (this._fxSnowLvl > 0) {
      ctx.save();
      ctx.beginPath(); ctx.rect(0, 0, W, fxH); ctx.clip();
      this._drawSnow(ctx, W, fxH, now);
      ctx.restore();
    }
  ...
  // Met le BLOC COMPLET a l'echelle (Chris : pas les elements un par un).
  ...
  _applyScale() {
    ...
    // wrapper. Signale par Chris (2026-08-17) : "les previsions ont disparu".
  ...
  _render() {
    ...
    else if (this._gl) this._skyStop();
    ...
    // Chris (2026-08-17) : « pour la pluie en dessous de 50% on se contentera des gouttes
    ...
    // la règle ne s'appliquait plus et Chris voyait les gouttes ET l'averse. On ne garde
  ...
  _skyInit() {
    ...
    const gl = cv.getContext('webgl', {
      ...
      // preserveDrawingBuffer:true -- OBLIGATOIRE depuis la plaque de fond de la
      // pluie (cf _fxPlate). Le ciel a sa PROPRE boucle RAF : quand la passe FX
      // veut le recopier, on n'est plus dans la frame ou il a ete dessine, et un
      // buffer non conserve a deja ete efface a la composition -> plaque noire,
      // gouttes grises.
...
void main(){
  ...
  // (constate par Chris sur capture, 2026-08-17). On garde donc glow pour l alpha
  ...
  // un plafond a 60% de blanc (demande de Chris le 2026-08-18 : "sans lumiere du soleil
  // les nuages se voient plutot comme des ombres refletees par la lune").
  // ⚠️ Reference Tokyo (photo Chris, meme jour) : SEULS les nuages colles a la lune
  ...
  // a la trappe. C est exactement le defaut que Chris reprochait deja a
  ...
  // ⚠️ LA NUIT, UN NUAGE EST OPAQUE (Chris, 2026-08-18 : "les nuages sont toujours
  ...
  // plein cadre -- l ancien voile, mais devenu son propre bouton (demande de Chris
  ...
  // valeur basse (ex 0.15, cas Chris), un ciel de nuit PEU couvert (cl faible car
  ...
  // transparente" (constate par Chris le 2026-08-18, cf reference images : nuit
...
`;
    ...
    const mk = (t, s) => {
      ...
      if (!gl.getShaderParameter(sh, gl.COMPILE_STATUS)) {
        console.error('[weather-neon-card-webgl] shader:', gl.getShaderInfoLog(sh));
        return null;
      }
      return sh;
    };
    ...
    if (!gl.getProgramParameter(pr, gl.LINK_STATUS)) {
      console.error('[weather-neon-card-webgl] link:', gl.getProgramInfoLog(pr));
      return;
    }
    gl.useProgram(pr);
    ...
    this._gl = gl; this._glPr = pr; this._glBuf = buf;
    this._glU = {};
    for (const n of ['uRes', 'uTime', 'uHorizon', 'uCover', 'uScale', 'uThick', 'uSpeed',
                     'uDir', 'uRelief', 'uDusk', 'uHalo', 'uHaze', 'uDepth', 'uSat',
                     'uGrain', 'uOpacity', 'uSunAlt', 'uLumX', 'uGrey', 'uTint',
                     'uVeil', 'uNightLit', 'uNightFloor', 'uNightSpan']) {
      this._glU[n] = gl.getUniformLocation(pr, n);
    }
    // Perte de contexte : Chrome la provoque en veille/onglet arriere. Sans ce
    // handler la card reste noire au reveil, sans un mot dans la console.
    // ⚠️ _skyStop(false) : SURTOUT ne pas remplacer le canvas ici. Le remplacer
    // (cloneNode) donne un noeud SANS listener, et le 'webglcontextrestored' qui
    // suit est alors emis sur l'ancien noeud detache -> jamais entendu, contexte
    // jamais reconstruit, card blanche jusqu'au F5. C'etait LE bug.
    cv.addEventListener('webglcontextlost', (e) => { e.preventDefault(); this._skyStop(false); }, false);
    cv.addEventListener('webglcontextrestored', () => { this._gl = null; this._skyInit(); }, false);

    this._skyT0 = performance.now();
    this._skyLoop();
  }

  // hard=true : demontage VOLONTAIRE (detachement, mode edition) -> on remplace le
  // canvas pour repartir d'un noeud vierge. hard=false : Chrome vient d'evincer le
  // contexte -> on GARDE le noeud, ses listeners, et on laisse 'webglcontextrestored'
  // faire son travail. Confondre les deux = card blanche definitive.
  _skyStop(hard = true) {
    if (this._skyRAF) { cancelAnimationFrame(this._skyRAF); this._skyRAF = null; }
    const gl = this._gl, cv = this._elSkyGl;
    if (gl && hard) {
      // liberer explicitement : un dashboard HA cree/detruit les cards a chaque
      // changement de vue, et les contextes WebGL sont plafonnes (~16 par onglet).
      if (this._glPr) gl.deleteProgram(this._glPr);
      if (this._glBuf) gl.deleteBuffer(this._glBuf);
      const ext = gl.getExtension('WEBGL_lose_context');
      if (ext) ext.loseContext();
      // ⚠️ Un canvas dont le contexte a ete perdu ne redonne JAMAIS un contexte
      // neuf : getContext() rend le MEME, toujours perdu. Or HA detache/rattache
      // la card en mode edition et on doit pouvoir repartir. On remplace donc le
      // noeud par un vierge (cloneNode(false) garde la classe, pas le bitmap).
      if (cv && cv.parentNode) {
        const n = cv.cloneNode(false);
        cv.parentNode.replaceChild(n, cv);
        this._elSkyGl = n;
      }
    }
    this._gl = null; this._glPr = null; this._glBuf = null; this._glU = null;
  }
  ...
  _skyLoop() {
    ...
    if (this._fitCanvas(cv)) gl.viewport(0, 0, cv.width, cv.height);
    // Effacement EXPLICITE. Ce calque tournait sans clear parce qu'un buffer non
    // conserve est vide a chaque frame -- le compositeur le nettoyait pour nous.
    // preserveDrawingBuffer:true (impose par la plaque de pluie, cf _fxPlate) supprime
    // ce nettoyage : sans ce clear, les frames s'ACCUMULENT et le rendu vire au lavis
    // rose sature au bout de quelques secondes.
    ...
    // ⚠️ LA NUIT SE MESURE, ELLE NE SE CALCULE PAS (Chris, 2026-08-18 : "le capteur est
  ...
  _fxGlInit() {
    ...
    this._fxGl = gl; this._fxBuf = buf; this._fxProgs = {};
    this.setAttribute('fxgl', '');          // masque le canvas 2D (CSS :host([fxgl]))

    // cf _skyStop : false = eviction Chrome, on garde le noeud et ses listeners.
    cv.addEventListener('webglcontextlost', (e) => { e.preventDefault(); this._fxGlStop(false); }, false);
    cv.addEventListener('webglcontextrestored', () => { this._fxGl = null; this._fxGlInit(); }, false);
  }

  // meme contrat que _skyStop : hard=false quand c'est Chrome qui evince.
  _fxGlStop(hard = true) {
    const gl = this._fxGl, cv = this._elFxGl;
    this.removeAttribute('fxgl');           // le 2D redevient visible : jamais d'ecran vide
    if (gl && hard) {
      for (const k in this._fxProgs) if (this._fxProgs[k]) gl.deleteProgram(this._fxProgs[k].pr);
      if (this._fxBuf) gl.deleteBuffer(this._fxBuf);
      for (const t of (this._fxTex || [])) gl.deleteTexture(t);
      const ext = gl.getExtension('WEBGL_lose_context');
      if (ext) ext.loseContext();
      // meme raison que _skyStop : un canvas dont le contexte est perdu ne redonne
      // JAMAIS un contexte neuf -- on remplace le noeud.
      if (cv && cv.parentNode) {
        const n = cv.cloneNode(false);
        cv.parentNode.replaceChild(n, cv);
        this._elFxGl = n;
    ...
    this._fxGl = null; this._fxBuf = null; this._fxProgs = null;
    this._fxTex = null; this._fxBlurCv = null; this._fxPlateCv = null;
    this._fxGrad = null; this._fxGradCss = null;
    this._fxMixUp = null;   // les textures sont mortes : forcer le re-upload de la mixmap
  }
  ...
  _fxGlEnsure() {
    ...
    const need = A.rain || A.fog || A.wind || A.frost || A.heat;
  ...
  _fxProgram(key, defs) {
    const gl = this._fxGl;
    if (this._fxProgs[key]) return this._fxProgs[key];
    const head = defs.map(d => '#define ' + d).join('\n') + '\n';
    const mk = (t, s) => {
      const sh = gl.createShader(t); gl.shaderSource(sh, s); gl.compileShader(sh);
      if (!gl.getShaderParameter(sh, gl.COMPILE_STATUS)) {
        console.error('[weather-neon-card-webgl] fx shader:', gl.getShaderInfoLog(sh));
        return null;
      }
      return sh;
    };
    const vs = mk(gl.VERTEX_SHADER, WeatherNeonCardWebgl.FX_VS);
    const fs = mk(gl.FRAGMENT_SHADER, head + WeatherNeonCardWebgl.FX_FS);
    if (!vs || !fs) { this._fxProgs[key] = null; return null; }
    const pr = gl.createProgram();
    gl.attachShader(pr, vs); gl.attachShader(pr, fs); gl.linkProgram(pr);
    if (!gl.getProgramParameter(pr, gl.LINK_STATUS)) {
      console.error('[weather-neon-card-webgl] fx link:', gl.getProgramInfoLog(pr));
      this._fxProgs[key] = null; return null;
    }
    const U = {};
    for (const n of WeatherNeonCardWebgl.FX_UNAMES) U[n] = gl.getUniformLocation(pr, n);
    this._fxProgs[key] = { pr, U };
    return this._fxProgs[key];
  }
  ...
  //    gris-bleu du shader -- et le constat de Chris du 2026-08-17, "des trucs gris
  ...
  _fxPlate(src) {
    ...
    // 3. le ciel WebGL (nuages, halo, crepuscule). preserveDrawingBuffer:true rend
    //    ce drawImage licite hors de la frame ou le ciel a ete dessine.
    ...
    // 5. l'averse en dernier : Chris veut "un peu de pluie derriere la vitre",
  ...
  _fxGlDraw(now) {
    ...
    const c = this._config;
    const A = this._fxActive();
    if (!A.rain && !A.fog && !A.wind && !A.frost && !A.heat) {
    ...
    const ap = gl.getAttribLocation(G.pr, 'p');
    gl.bindBuffer(gl.ARRAY_BUFFER, this._fxBuf);
    ...
    u1('uWindWarp', c.fx_vent_warp * wf); u1('uWindTurb', c.fx_vent_turb);
    u1('uWindSwirl', c.fx_vent_swirl);    u1('uWindTint', c.fx_vent_teinte);
  ...
  _moonInit() {
    ...
    if (!gl) return;                       // pas de WebGL -> le SVG reprend la main
    // Contexte encore perdu : getContext() rend le MEME contexte mort, les shaders ne
    // compileraient pas et on cracherait un console.warn a chaque render. On sort propre ;
    // `webglcontextrestored` (ou le prochain _moonEnsure) reessaiera.
    if (gl.isContextLost()) return;
    const sh = (t, s) => {
      const o = gl.createShader(t); gl.shaderSource(o, s); gl.compileShader(o);
      if (!gl.getShaderParameter(o, gl.COMPILE_STATUS)) {
        console.warn('[weather-neon-webgl] lune :', gl.getShaderInfoLog(o)); return null;
      }
      return o;
    };
    const vs = sh(gl.VERTEX_SHADER, WeatherNeonCardWebgl.MOON_VS);
    const fs = sh(gl.FRAGMENT_SHADER, WeatherNeonCardWebgl.MOON_FS);
    if (!vs || !fs) return;
    const pr = gl.createProgram();
    gl.attachShader(pr, vs); gl.attachShader(pr, fs); gl.linkProgram(pr);
    if (!gl.getProgramParameter(pr, gl.LINK_STATUS)) {
      console.warn('[weather-neon-webgl] lune :', gl.getProgramInfoLog(pr)); return;
    }
    gl.useProgram(pr);
    const buf = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, buf);
    ...
    const U = {};
    WeatherNeonCardWebgl.MOON_UN.forEach(n => { U[n] = gl.getUniformLocation(pr, n); });
    gl.enable(gl.BLEND);
    gl.blendFunc(gl.ONE, gl.ONE_MINUS_SRC_ALPHA);   // sortie premultipliee
    this._moonGl = gl; this._moonProg = pr; this._moonBuf = buf; this._moonU = U;
    ...
    // le contexte peut etre repris par le navigateur (onglet en arriere-plan, GPU
    // sature) : sans ces deux handlers la lune disparait jusqu'au F5.
    cv.addEventListener('webglcontextlost', e => {
      e.preventDefault(); this._moonSnap = 'lost';
      this._moonGl = null;                  // sinon _moonInit() ressort par sa propre garde
      this.removeAttribute('moongl');       // le SVG reprend la main en attendant
    }, { passive: false });
    cv.addEventListener('webglcontextrestored', () => {
      this._moonGl = null; this._moonInit(); this._moonDraw();
    });

    // la texture arrive de facon asynchrone (decodage JPEG) -- on redessine a l'arrivee
    const img = new Image();
    img.onload = () => {
      if (!this._moonGl) return;
      const g = this._moonGl, t = g.createTexture();
      g.bindTexture(g.TEXTURE_2D, t);
      g.pixelStorei(g.UNPACK_FLIP_Y_WEBGL, true);
      g.texImage2D(g.TEXTURE_2D, 0, g.LUMINANCE, g.LUMINANCE, g.UNSIGNED_BYTE, img);
  ...
  _moonEnsure() {
    ...
    if (!this._moonOn && this._moonGl) { this._moonStop(); return; }
    if (!this._moonGl && this._moonOn) {
      // Un canvas dont le contexte est perdu ne peut PAS en redonner un neuf tant que Chrome
      // ne le restaure pas -- et sur une EVICTION (changement de vue) il ne le restaure
      // jamais : `webglcontextrestored` ne tire que sur reset GPU. Seule issue : jeter le
      // canvas et en poser un vierge. `_moonSnap` borne la manoeuvre a UNE fois par perte,
      // sinon on remplacerait le canvas a chaque render sur une machine sans WebGL.
      const cv = this._elMoonGl;
      if (this._moonSnap === 'lost' && cv) {
        this._moonSnap = '';
        const fresh = cv.cloneNode(false);   // false : pas d'enfants, pas d'ecouteurs
        cv.replaceWith(fresh);
        this._elMoonGl = fresh;
      }
      this._moonInit();
    }
    this._moonDraw();
  ...
  _moonStop() {
    ...
    if (!gl) return;
    if (this._moonTex) gl.deleteTexture(this._moonTex);
    if (this._moonProg) gl.deleteProgram(this._moonProg);
    if (this._moonBuf) gl.deleteBuffer(this._moonBuf);
    const lose = gl.getExtension('WEBGL_lose_context');
    if (lose) lose.loseContext();
    this._moonGl = null; this._moonProg = null; this._moonBuf = null;
  ...
  // ── NEIGE (canvas 2D, DANS la texture de scene) ───────────────────────────
  ...
  //    quelque chose depuis chez Chris (campagne, zero pollution lumineuse) :
  ...
  _aurInit() {
    ...
    const gl = cv.getContext('webgl', {
      alpha: true, premultipliedAlpha: true, antialias: false,
      depth: false, stencil: false, powerPreference: 'low-power',
      preserveDrawingBuffer: true,   // meme raison que le ciel, cf _fxPlate
    });
...
vec3 nappe(vec2 uv, float seed){
  ...
  // ⚠️ LE BLEU NE SUIT PAS LE PROFIL DU FAISCEAU. Vecu le 2026-08-17 (Chris : "le
...
`;
    const mk = (t, s) => {
      const sh = gl.createShader(t); gl.shaderSource(sh, s); gl.compileShader(sh);
      if (!gl.getShaderParameter(sh, gl.COMPILE_STATUS)) {
        console.error('[weather-neon-card-webgl] aurore shader:', gl.getShaderInfoLog(sh));
        return null;
      }
      return sh;
    };
    ...
    gl.attachShader(pr, vs); gl.attachShader(pr, fs); gl.linkProgram(pr);
    if (!gl.getProgramParameter(pr, gl.LINK_STATUS)) {
      console.error('[weather-neon-card-webgl] aurore link:', gl.getProgramInfoLog(pr));
      return;
    }
    gl.useProgram(pr);
    const buf = gl.createBuffer();
    ...
    gl.vertexAttribPointer(loc, 2, gl.FLOAT, false, 0, 0);
    gl.enable(gl.BLEND);
    gl.blendFunc(gl.ONE, gl.ONE_MINUS_SRC_ALPHA);   // shader premultiplie, comme le ciel

    this._aurGl = gl; this._aurPr = pr; this._aurBuf = buf; this._aurU = {};
    for (const n of ['uRes', 'uTime', 'uBase', 'uAmp', 'uSigma', 'uPlis', 'uFin',
                     'uDerive', 'uOndul', 'uVert', 'uRouge', 'uBleu', 'uNappes',
                     'uLargeur', 'uCentre', 'uOpacity', 'uPulse']) {
      this._aurU[n] = gl.getUniformLocation(pr, n);
    }
    // meme doctrine que le ciel : sur perte de contexte on GARDE le noeud et ses
    // listeners, sinon le 'restored' part sur un noeud detache et n'est jamais entendu.
    cv.addEventListener('webglcontextlost', (e) => { e.preventDefault(); this._aurStop(false); }, false);
    cv.addEventListener('webglcontextrestored', () => { this._aurGl = null; this._aurInit(); }, false);
  ...
  // hard=true : extinction VOLONTAIRE (verdict tombe, demontage) -> noeud remplace.
  // hard=false : Chrome vient d'evincer le contexte -> on garde le noeud.
  _aurStop(hard = true) {
    if (this._aurRAF) { cancelAnimationFrame(this._aurRAF); this._aurRAF = null; }
    const gl = this._aurGl, cv = this._elAurGl;
    if (gl && hard) {
      if (this._aurPr) gl.deleteProgram(this._aurPr);
      if (this._aurBuf) gl.deleteBuffer(this._aurBuf);
      const ext = gl.getExtension('WEBGL_lose_context');
      if (ext) ext.loseContext();
      // un canvas dont le contexte est perdu ne redonne jamais un contexte neuf
      if (cv && cv.parentNode) {
        const n = cv.cloneNode(false);
        cv.parentNode.replaceChild(n, cv);
        this._elAurGl = n;
    ...
    this._aurGl = null; this._aurPr = null; this._aurBuf = null; this._aurU = null;
  ...
  _aurEnsure() {
    ...
    else if (this._aurGl) this._aurStop();
  }
  ...
  _aurLoop() {
    ...
    if (gl.isContextLost()) { this._aurStop(); return; }
...
WeatherNeonCardWebgl.styles = `
  ...
  /* 50px -> 44px (Chris, 2026-08-17, vu sur telephone) : la temperature poussait la
     colonne de droite au point de tronquer le nom de la commune
     ("BURTHECOURT-AUX-CHÊ…"). On lui rend ~15 px sans qu'elle cesse d'etre l'element
...
uniform float uWindWarp,uWindTurb,uWindSwirl,uWindTint;
...
#endif
#ifdef FX_WIND
  vec3 wq=vec3(uv.x*uWindTurb*0.45 - uTime*0.45, uv.y*uWindTurb*0.45, uTime*0.20);
...
class WeatherNeonCardWebglEditor extends HTMLElement {
  ...
  _schema() {
    ...
    this._group('Givre', false, () => {
      ...
      this._number('fx_givre_lis', 'Lissage', { min: 0, max: 2, step: 0.05, ph: '0.80' });
      this._number('fx_givre_seuil', 'Seuil', { min: 0, max: 1, step: 0.01, ph: '0.06' });
