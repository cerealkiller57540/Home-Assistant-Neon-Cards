...
function nmcRenderNodes(nodes, hass, vars, errs) {
  let out = "";
  for (const n of nodes) {
    ...
    if (n.t === "expr") {
      try {
        ...
        out += nmcToStr(nmcApplyFilters(nmcEval(expr, hass, vars), filters, hass, vars));
      } catch (e) {
    ...
    if (n.t === "for") {
      ...
      for (let i = 0; i < len; i++) {
        ...
        out += nmcRenderNodes(n.body, hass, child, errs);
      }
      continue;
    }
    if (n.t === "if") {
      for (const br of n.branches) {
        if (br.cond === null) {
          out += nmcRenderNodes(br.body, hass, vars, errs);
          break;
        }
        let ok = false;
        try {
...
function nmcEval(expr, hass, vars) {
  ...
  }
  {
    ...
    if (inM) {
      const val = nmcToStr(nmcEvalF(inM.val, hass, vars));
      const items = inM.list
        .replace(/^\[|\]$/g, "")
        .split(",")
        .map((x) => {
          x = x.trim();
...
function nmcSafeSrc(raw) {
  const r = nmcSafeHref(raw);
  if (r.safe) return r;
  if (raw && /^\/(local|api|www|media|brand)\//i.test(String(raw).trim()))
...
class NeonMarkdownCard extends HTMLElement {
  ...
  _updateBody() {
    ...
    if (mode === "body" || mode === "both") {
      ...
      if (el) {
        ...
        if (el.innerHTML !== html) el.innerHTML = html; // ne réécrire que si changé (pas de reset d'anim)
      }
    }
  }
  ...
  _render() {
    ...
    const tFontFamily = t.font_family
      ? `'${t.font_family}', var(--primary-font-family, sans-serif)`
    ...
    const bFontFamily = b.font_family
      ? `'${b.font_family}', var(--primary-font-family, sans-serif)`
      : tFontFamily;
