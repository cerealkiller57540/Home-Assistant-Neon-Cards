...
from .py_netgear_plus_local.fetcher import (
    LoginFailedError,
...
async def async_setup_entry(
    ...
    async def async_update_switch_infos() -> dict[str, Any] | None:
        ...
        try:
            ...
            raw_response = gs_switch.get_last_raw_response()
            if raw_response:
                _LOGGER.debug(
                    "Raw Netgear response for %s after update failure:\n%s",
                    entry.data[CONF_HOST],
                    raw_response,
                )
            raise UpdateFailed(
                f"Unable to update {gs_switch.device_name}: {error}"
            ) from error
