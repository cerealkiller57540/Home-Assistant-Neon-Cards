...
class NetgearSwitchConnector:
    ...
    def get_login_cookie(self) -> bool:
        ...
        # The GS108T has NO logout endpoint (its LOGOUT_TEMPLATES entry is just
        # a GET on the login page — verified: the SID still works afterwards).
        # So only drop the cookie locally; asking the switch to log us out is
        # a pointless extra request.
        if self._page_fetcher.get_cookie() != (None, None):
            _LOGGER.debug(
                "[NetgearSwitchConnector.get_login_cookie] "
                "stale cookie present, dropping it before re-login"
            )
            self._page_fetcher.clear_cookie()

        # The GS108T login has no challenge value. Re-authenticate directly
        # instead of probing /, which can intermittently stop responding on
        # this old firmware while the login endpoint is still available.
        if (
            self.switch_model.CRYPT_FUNCTION != "none"
            and not self._page_fetcher.get_login_page_response()
        ):
            self._page_fetcher.check_login_url(self.switch_model)
        rand = (
            self._page_parser.parse_login_form_rand(
                self._page_fetcher.get_login_page_response()
            )
            if self.switch_model.CRYPT_FUNCTION != "none"
            else None
        )
    ...
    def get_last_raw_response(self) -> str:
        ...
        return self._page_fetcher.get_last_raw_response()
        ...
        for attempt in range(2):
            for template in templates:
                ...
                if self._page_fetcher.has_ok_status(response):
                    return response
            if attempt == 0 and not self.get_offline_mode():
                _LOGGER.debug(
                    "[fetch_page_from_templates] page unavailable, "
                    "retrying with existing session after 0.5s"
                )
                time.sleep(0.5)
    ...
    def get_switch_infos(self) -> dict[str, Any]:
        ...
        # Do NOT log out at the end of a poll. The GS108T has no logout
        # endpoint, so "logging out" only dropped our cookie locally while the
        # session stayed alive on the switch. That forced a brand new login
        # every 60s, each one leaking a session, until the switch hit its
        # concurrent-session cap and stopped handing out SIDs at all.
        # Keep ONE sticky session and only re-login when a page comes back
        # unauthenticated (handled in fetch_page_from_templates).
        ...
        if not self.get_offline_mode():
            sample_time = sample_timestamp - self._previous_timestamp
        else:
            sample_time = 0
        ...
        switch_data["response_time_s"] = round(
            time.perf_counter() - cycle_start_time, 1
        )
    ...
    def _update_current_data(
        ...
        for port_number0 in range(self.ports):
            try:
                ...
                _first_poll = "traffic_rx" not in self._previous_data
                current_data[f"port_{port_number}_traffic_rx"] = (
                    0 if _first_poll else
