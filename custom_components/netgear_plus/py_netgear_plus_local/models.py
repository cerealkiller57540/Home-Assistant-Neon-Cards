...
class GS108T(AutodetectedSwitchModel):
    """Definition for Netgear GS108T / GS108Tv2 managed switch (8-port L2)."""

    MODEL_NAME = "GS108T"
    PORTS = 8
    POE_PORTS: ClassVar = []

    # Login page has <TITLE>NETGEAR GS108T</TITLE> and no rand input field
    CHECKS_AND_RESULTS: ClassVar = [
        ("check_login_form_rand", [False]),
        ("parse_login_title_tag", ["GS108T"]),
    ]

    CRYPT_FUNCTION: ClassVar = "none"

    AUTODETECT_TEMPLATES: ClassVar = [
        {"method": "get", "url": "http://{ip}/"},
    ]

    # Plain password POST, no hash — maps pwd field to _password_hash attribute
    LOGIN_TEMPLATE: ClassVar = {
        "method": "post",
        "url": "http://{ip}/base/main_login.html",
        "params": {"pwd": "_password_hash"},
    }

    SWITCH_INFO_TEMPLATES: ClassVar = [
        {"method": "get", "url": "http://{ip}/base/system/management/sysInfo.html"},
    ]

    PORT_STATUS_TEMPLATES: ClassVar = [
        {"method": "get", "url": "http://{ip}/base/system/port/port_cfg.html"},
    ]

    PORT_STATISTICS_TEMPLATES: ClassVar = [
        {"method": "get", "url": "http://{ip}/base/system/stats/statsport_summ.html"},
    ]

    LOGOUT_TEMPLATES: ClassVar = [
        {"method": "get", "url": "http://{ip}/base/main_login.html"},
    ]
