...
const EYES = {
  open:  [0b10010001001, 0b10101010101],  // pics centraux haut · côtés bas → \/
  half:  [0b10000000001, 0b10111011101],  // r4 vide · r5 barre fine
  close: [0b10111011101, 0b10111011101],  // double barre fermée
};
...
function _sleepyDots(accent) {
  ...
  // Off dots pour les rangées yeux aussi (fond)
  for (let ry = 4; ry <= 5; ry++) {
    for (let c = 0; c < nC; c++) {
      baseOff += `<circle cx="${(pad+c*step*sc+r*sc).toFixed(1)}" cy="${(pad+ry*step*sc+r*sc).toFixed(1)}" r="${rsc}"/>`;
    }
  }
...
class StoreyBatteryCard extends HTMLElement {
  ...
  _buildKey() {
    ...
    const pwKey = pw !== null
      ...
      : null;
    return `${m}|${soc!==null?Math.round(soc):null}|${pwKey}|${msv}|${mods}|${this._config.color_accent||''}|${this._config.color_bg||''}|${this._config.glow_enabled||''}|${this._config.cyberpunk_mode||''}|${this._config.neon_glow||''}|${this._config.card_mod_bg||''}|${thr}`;
  }
  ...
  _render() {
    ...
    if (pwVal !== null || masterState !== null) {
      ...
      const { circles: arrowC, bW: arrowBW, bH: arrowBH } = _arrowDots(isIdle ? true : isCharging, dirCol, accent);
      const lblArrow = isIdle ? 'IDLE' : (masterState ? masterState : (isCharging ? 'CHARGING' : 'DISCHARGING'));
      panelsSVG +=
    ...
    if (!root.querySelector('ha-card')) {
      root.innerHTML = `<style>
        ...
        * { box-sizing: border-box; font-family: -apple-system, 'SF Pro Text', 'Helvetica Neue', sans-serif }
        ...
        .cp-tl { top:0; left:0; border-top: 2px solid var(--sbc-cp-primary,${CP_PRIMARY}); border-left: 2px solid var(--sbc-cp-primary,${CP_PRIMARY}) }
        .cp-br { bottom:0; right:0; border-bottom: 2px solid var(--sbc-cp-primary,${CP_PRIMARY}); border-right: 2px solid var(--sbc-cp-primary,${CP_PRIMARY}) }
        ...
        :host([data-neon]) .brand { text-shadow: 0 0 8px #fff, 0 0 20px var(--sbc-accent), 0 0 40px var(--sbc-accent) }
        .kwh-pill {
